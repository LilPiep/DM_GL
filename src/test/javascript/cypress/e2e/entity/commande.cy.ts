...
describe('Commande e2e test', () => {
  ...
  const commandeSample = { commandeID: 67739, meansOfPayment: 'Visa' };
  ...
  describe('new Commande page', () => {
    ...
    it('should create an instance of Commande', () => {
      cy.get(`[data-cy="commandeID"]`).type('53606').should('have.value', '53606');

      cy.get(`[data-cy="meansOfPayment"]`).select('ChequeRepas');

      cy.get(`[data-cy="deadline"]`).type('2023-04-09T06:50').blur().should('have.value', '2023-04-09T06:50');

      cy.get(`[data-cy="price"]`).type('1536').should('have.value', '1536');
