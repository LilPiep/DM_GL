...
/**
 * REST controller for managing {@link polytech.info.gl.domain.Livreur}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class LivreurResource {
    ...
    public ResponseEntity<Livreur> partialUpdateLivreur(
        ...
        Optional<Livreur> result = livreurRepository
            ...
            .map(existingLivreur -> {
                if (livreur.getLivreurName() != null) {
                    existingLivreur.setLivreurName(livreur.getLivreurName());
                }
                if (livreur.getLivreurID() != null) {
                    existingLivreur.setLivreurID(livreur.getLivreurID());
                }
